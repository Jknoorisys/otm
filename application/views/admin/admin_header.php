		...
		<div id="veticaluser" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="vcenter" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form class="mt-4" action="<?= base_url('admin-add-user') ?>" method="POST">
							<div class="row">
								<div class="col-12">
									<label class="mr-sm-2" for="">Select Team Lead</label>
									<select class="custom-select mr-sm-2" name="tl_id">
										<option selected value="">Choose...</option>
										<?php foreach ($tls as $user) { ?>
...
<script>
	function changeController(selectObject) {
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url('admin-get-user-balance-leave') ?>",
